...
import { Empleado, GastoCorriente, GastoCorrienteTable } from '@/types';
import { IconPlus } from '@tabler/icons-react';
import { useEffect, useState } from 'react';
import { useForm } from 'react-hook-form';
import toast from 'react-hot-toast';

interface AddGastoCorrienteModalProps {
    onGastoCreado: (gasto: GastoCorrienteTable) => void;
    open?: boolean;
    onOpenChange?: (open: boolean) => void;
}

export default function AddGastoCorrienteModal({ onGastoCreado, open, onOpenChange }: AddGastoCorrienteModalProps) {
    ...
    const onSubmit = async (data: GastoCorriente) => {
        try {
            const csrfToken = document.querySelector('meta[name="csrf-token"]')?.getAttribute('content');
            const response = await fetch('/GastosCorrientes', {
                method: 'POST',
                headers: {
                    ...
                    <form onSubmit={form.handleSubmit(onSubmit)} className="grid gap-4 py-2">
                        {[
                            ...
                            <FormField
                                ...
                                name={field.name as keyof GastoCorriente}
